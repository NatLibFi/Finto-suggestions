...
class GithubDataParser:
    ...
    def __parse_preferred_labels(self, value, body):
        ...
        if 'preflabel' in value:
            ...
            splitted_parsed_value = splitted_values[1].replace(
                '[', '').replace(']', '').split('(')
    ...
    def __parse_broader(self, value, body):
        ...
        if len(splitted_value) > 0:
            splitted_broader = splitted_value[1].replace(
                '(', '').replace(')', '').split('[')[1:]
        ...
        for value in splitted_matches:
            if len(value) > 0:
                ...
                if 'http' in value:
                    ...
                    match_value = ''
                body.exact_matches.append(
                    {'vocab': match_vocab, 'value': match_value})
    ...
    def __parse_remove_name_and_email_from_body(self, body_str):
        ...
        if 'Ehdottajan sähköpostiosoite' in body_str:
            split_remove_sender_email = body_str.split(
    ...
    def __parse_remove_yse_term_from_body(self, body_str):
        if 'Termiehdotus Fintossa' in body_str:
            ...
            body_str = split_remove_yse_term[0].replace(
                '*', '').replace(':', '').strip()
        return body_str
    ...
    def __parse_remove_org_and_term_suggestion_from_reason(self, value):
        if 'Ehdottajan organisaatio' in value:
            ...
            parsed_value = splitted_remove_org[0].replace(
                '*', '').replace(':', '').strip()
            return parsed_value
        if 'Termiehdotus Fintossa' in value:
            splitted_remove_term_on_finto = value.split(
                'Termiehdotus Fintossa')
            parsed_value = splitted_remove_term_on_finto[0].replace(
                '*', '').strip()
            return parsed_value

    def __parse_reason(self, value, body):
        if 'Aineisto jonka kuvailussa käsitettä tarvitaan (esim. nimeke tai URL)' in value:
            splitted_reasons = value.split(
                'Aineisto jonka kuvailussa käsitettä tarvitaan (esim. nimeke tai URL)')
            parsed_value = self.__parse_remove_org_and_term_suggestion_from_reason(
                splitted_reasons[1].strip())
            if parsed_value is None:
                body.reason = splitted_reasons[1].strip()
            else:
                body.reason = parsed_value
        if 'Perustelut ehdotukselle' in value:
            splitted_reason_description = value.split(
                'Perustelut ehdotukselle')
            parsed_value = self.__parse_remove_org_and_term_suggestion_from_reason(
                splitted_reason_description[1].strip())
            if parsed_value is None:
    ...
    def __parse_groups(self, value, body):
        splitted_value = value.split(
            'Ehdotetut temaattiset ryhmät (YSA-ryhmät)')[1].split('**')[0]
        if len(splitted_value) > 0:
            splitted_groups = splitted_value.replace(
                '(', '').replace(')', '').split('[')[1:]
    ...
    def __parse_organization(self, value, body):
        ...
        if 'Ehdottajan organisaatio' in value:
            ...
            organization = splitted_value[1].replace(
        ...
        if 'Termiehdotus Fintossa' in organization:
            ...
            organization = splitted_organization[0].replace(
                '*', '').replace(':', '').strip().split('####')[0]
...
# Marker 1
    ...
    def __map_response(self, json_item):
        ...
        if json_item["comments"] > 0:
            response = self.__fetch_comments_from_github(
                json_item["comments_url"])
    ...
    def handle_response(self, arg_loop_count):
        ...
        if arg_loop_count is not None and arg_loop_count > 0:
            ...
            loop_count = self.__parse_count_from_response_headers(
                response.headers)
