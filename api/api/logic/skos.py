...
from rdflib import Graph, URIRef, Literal, Namespace, RDF, XSD, term
...
def suggestionToGraph(suggestion, graph=None):
    ...
    try:
        if suggestion is not None:
            ...
            try:
                for tag in suggestion["tags"]:
                    if 'maantieteellinen' in tag["label"].lower():
                        g.add(((URIRef(uri), RDF.type, URIRef(
                            ysometa + 'GeographicalConcept'))))
            except Exception as ex:
                print(str(ex))

            try:
                suggestion_system_url = os.environ.get(
                    'SUGGESTION_SYSTEM_ISSUE_URL', None)
            except Exception as ex:
                print(str(ex))
            ...
            try:
                g.add((URIRef(uri), foaf.homepage, URIRef(
                    f'{suggestion_system_url}/api/suggestions/{suggestion["id"]}/skos')))
            except Exception as ex:
                print(str(ex))

            try:
                g.add((URIRef(uri), dct.created, Literal(
                    suggestion["created"].date(), datatype=XSD.date)))
                g.add((URIRef(uri), dct.modified, Literal(
            ...
            try:
                for lang in ("fi", "sv", "en"):
                    if lang in suggestion["preferred_label"] and suggestion["preferred_label"][lang].get('value'):
                        g.add((URIRef(uri), skos.prefLabel, Literal(
            ...
            try:
                for group in suggestion["groups"]:
                    if group.get('uri'):
                        g.add((URIRef(uri), skos.member,
                               term.URIRef(group.get('uri'))))

                for match in suggestion["broader_labels"]:
                    if match.get('uri'):
                        g.add((URIRef(uri), skos.broadMatch,
                               URIRef(match.get('uri'))))

                if suggestion.get('scopeNote'):
                    g.add((URIRef(uri), skos.scopeNote, Literal(
                        suggestion["scopeNote"].strip())))

                for aLabel in suggestion["alternative_labels"]:
                    if aLabel.get('value'):
                        g.add((URIRef(uri), skos.altLabel,
                               Literal(aLabel["value"].strip())))

                for match in suggestion["narrower_labels"]:
                    if match.get('uri'):
                        g.add((URIRef(uri), skos.narrowMatch,
                               URIRef(match.get('uri'))))

                for match in suggestion["related_labels"]:
                    if match.get('uri'):
                        g.add((URIRef(uri), skos.relatedMatch,
                               URIRef(match.get('uri'))))
            except Exception as ex:
                print(str(ex))
